...
align_audio:
	python scripts/align_audio.py --tokens "$(TOKENS)" --highlighter "$(HIGHLIGHTER)" --audio "$(AUDIO)" --out "$(TIMINGS)" --engine "$(ENGINE)" --url "$(URL)" $(if $(DURATION_MS),--duration-ms "$(DURATION_MS)",)
